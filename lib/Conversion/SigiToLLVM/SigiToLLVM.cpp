...
static bool isSigiClosureType(Type ty)
{
    ...
    if (auto boxTy = ty.dyn_cast<closure::BoxedClosureType>()) {
        auto funTy = boxTy.getFunctionType();
        return funTy.getInputs() == TypeRange{stackTy}
               && funTy.getResults() == TypeRange{stackTy};
    }
    return false;
}
    ...
    LLVM::LLVMFuncOp* resultFunc)
{
    ...
    */
    ...
    if (auto ptrTy = paramSrcTy.dyn_cast<LLVM::LLVMPointerType>())
        if (ptrTy.isOpaque()) return lookup("sigi_push_closure");
    ...
    LLVM::LLVMFuncOp* resultFunc)
{
    ...
    */
    ...
    if (auto ptrTy = resultSrcTy.dyn_cast<LLVM::LLVMPointerType>())
        if (ptrTy.isOpaque()) return lookup("sigi_pop_closure");
...
*/
        ...
        auto* block = newMainFunc.addEntryBlock();
        ...
        rewriter0.updateRootInPlace(op, [&]() { op->removeAttr("sigi.main"); });
        return success();
    }
};
